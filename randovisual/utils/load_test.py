...
def perturb_geometry(session, geometry, perturbation_scale, perturb_vertices=True):
    ...
    if wkt.startswith('LINESTRINGZ'):
        ...
        for point in points:
            ...
            if perturb_vertices:
                new_lon += random.uniform(-perturbation_scale * 0.3, perturbation_scale * 0.3)
                new_lat += random.uniform(-perturbation_scale * 0.3, perturbation_scale * 0.3)
    ...
    with Session(engine) as session:
        ...
        for i, original_route in enumerate(existing_routes):
            for copy_num in range(multiplier):
                ...
                rides = generate_rides_for_route(session, new_route, member_ids, num_rides)
        ...
        print(f"Total routes in DB: {session.scalar(select(func.count(Route.rusa_id)))}")
